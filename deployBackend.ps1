...
$imageName = "mathview-backend"
$imageTag = "latest"
$fullImageName = "${acrLoginServer}/${imageName}:${imageTag}"
$appServicePlan = "mathview-plan"
$webAppName = "mathview-backend"

# 1. Construir la imagen Docker sin caché
Write-Host "Construyendo imagen Docker del backend..."
docker build --no-cache -f ./dockerfile.api -t $fullImageName .
...
Write-Host "Reiniciando Web App para aplicar la nueva imagen..."
az webapp restart --name $webAppName --resource-group $resourceGroup

# 9. URL Final
Write-Host "`n✅ Backend desplegado. Accede a: https://$webAppName.azurewebsites.net"
